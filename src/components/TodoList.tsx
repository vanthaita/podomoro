...
const TodoList: React.FC<Props> = ({todoPosition, setTodoPosition, containerRef}) => {
    ...
    const handleTimeChange = (id: string, event: React.ChangeEvent<HTMLInputElement>) => {
        const newTime = parseInt(event.target.value);
        setTodos((prevTodos) =>
            prevTodos.map((todo) =>
                todo.id === id ? { ...todo, estimatedTime: isNaN(newTime) ? undefined : newTime } : todo
            )
        );
    };
    ...
    const calculateRemainingTime = (estimatedTime: number | undefined, timeElapsed: number | undefined) => {
    ...
    return (
        <div
            ...
            {...bind()}
            >
            <h2 className="text-lg font-semibold mb-4 dark:text-gray-100">Task List</h2>
                          ...
                          <AccordionTrigger
                            ...
                            >
                              <div className="flex items-center gap-2">
                                  <span
                                  className={cn("dark:text-gray-100",
                                   todo.completed ? "line-through text-gray-500 dark:text-gray-400" : undefined
                                   )}
                                   >{todo.text}
                                  </span>
                              </div>
                            ...
                            <div className="flex items-center justify-between mt-2">
                              <div className="flex gap-2 items-center">
                                ...
                                <Input
                                    ...
                                    value={todo.estimatedTime === undefined ? "" : todo.estimatedTime.toString()}
                                    onChange={(e) => handleTimeChange(todo.id, e)}
                                    disabled={todo.completed}
                                />
                                <span>{calculateRemainingTime(todo.estimatedTime, todo.timeElapsed)}</span>
                              </div>
                                 <Button
                                  variant="destructive"
                                  size="sm"
            ...
            {todos.some(todo => todo.completed) && (
               <div className="mt-4">
                  <h3 className="text-md font-semibold mb-2 dark:text-gray-100">Completed Tasks</h3>
                  <ul className="list-none pl-0">
                  {todos
                      .filter((todo) => todo.completed)
                      .map((todo) => (
                        <li key={todo.id} className="flex items-center justify-between py-2 group hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md transition-colors duration-200">
